...
class AppHeader extends Component {
  ...
  render() {
      ...
      let globalSearch = (this.props.settings.ui.header.useGlobalSearch) ? ( <
          ...
          Input type = "text"
          placeholder = "Search"
          isExpanded style = { styles.fullWidth }
          /> <
          /NavItem> <
          /NavGroup>) : null;
          let navLabelTitle = (!this.props.settings.ui.header.useGlobalSearch && this.props.ui.nav_label) ? ( <
              ...
              /NavItem>) : null;
              return ( <
                  Hero color = { this.props.settings.ui.header.color }
                  isBold = { this.props.settings.ui.header.isBold }
                  style = { Object.assign(styles.fixedTop, styles.navContainer, this.props.settings.ui.header.containerStyle) }
                  className = {
                    (this.props.settings.ui.initialization.show_header || this.props.user.isLoggedIn) ? 'animated fadeInDown Header-Speed' : 'animated slideOutDown Header-Speed' } > {
                    ...
                    this.getRenderedComponent(this.props.ui.components.header.layout) :
                      ...
                      NavItem > {
                        ...
                        this.getRenderedComponent(this.props.settings.ui.header.customButton) :
                          ( < Button onClick = { this.props.toggleUISidebar }
                          buttonStyle = "isOutlined"
                          color = { buttonColor }
                          icon = "fa fa-bars"
                          style = { styles.iconButton }
                          /> )} <
                          /NavItem> { navLabelTitle } <
                          /NavGroup> { globalSearch } <
                          NavGroup align = "right"
                          isMenu >
                          <
                          NavItem >
                          <
                          Link to = { `${this.all_prefixes.manifest_prefix}account/profile` }
                          style = { Object.assign({ fontSize: '20px', }, styles.noUnderline, this.props.settings.ui.header.userNameStyle) } > { `${capitalize(this.state.user.firstname || '')} ${capitalize(this.state.user.lastname || '')}` } <
                          /Link> <
                          /NavItem>  <
                          NavItem > {
                          ...
                          /NavItem> {
                            ...
                            ( < NavItem >
                              <
                              Button buttonStyle = "isOutlined"
                              onClick = { this.props.logoutUser }
                              color = { buttonColor }
                              icon = "fa fa-sign-out"
                              style = { Object.assign({ paddingRight: 0, }, styles.noMarginLeftRight) }
                              /> <
                              /NavItem>): null
                            } <
                            /NavGroup> <
