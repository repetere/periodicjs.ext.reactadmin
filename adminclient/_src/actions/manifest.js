...
var manifest = {
  ...
  fetchManifest: function fetchManifest() {
    ...
    var manifestAction = function manifestAction(dispatch, getState) {
      ...
      var state = getState();
      var hasCached = state.manifest && state.manifest.containers && state.manifest.hasLoaded;
      if (hasCached) dispatch(_this.receivedManifestData(state.manifest));
      ...
      return _util2.default.fetchComponent(basename + '/load/manifest' + (state.settings.ui.initialization.refresh_manifests ? '?refresh=true' : ''), options)().then(function (response) {
  ...
  fetchUnauthenticatedManifest: function fetchUnauthenticatedManifest() {
    ...
    var unauthenticatedManifestAction = function unauthenticatedManifestAction(dispatch, getState) {
      ...
      var state = getState();
      var hasCached = state.manifest && state.manifest.containers && state.manifest.unauthenticated.hasLoaded;
      if (hasCached) dispatch(_this2.unauthenticatedReceivedManifestData(state.manifest));
      var basename = typeof state.settings.adminPath === 'string' && state.settings.adminPath !== '/' ? state.settings.basename + state.settings.adminPath : state.settings.basename;
      //add ?refresh=true to below route to reload manifest configuration
      return _util2.default.fetchComponent(basename + '/load/public_manifest')().then(function (response) {
